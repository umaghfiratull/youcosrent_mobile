...
class PesananMyOrderView extends GetView<PesananMyOrderController> {
  PesananMyOrderView({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      if (controllerHistory.modelDataPesanan.data == null ||
          controllerHistory.modelDataPesanan.data!.isEmpty) {
        return Center(child: CircularProgressIndicator());
      }
      return Padding(
        padding: const EdgeInsets.only(top: 10, left: 10, right: 10, bottom: 5),
        child: ListView.builder(
          itemCount: controllerHistory.modelDataPesanan.data?.length ?? 0,
          itemBuilder: (context, index) {
            var order = controllerHistory.modelDataPesanan.data?[index];
            return Card(
              color: Colors.white,
